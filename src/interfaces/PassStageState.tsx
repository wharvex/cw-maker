...
export const getVCIdx = (pss: PassStageState): number => {
  let idx: number = Math.floor(Math.random() * pss.vcs.length);
  while (pss.triedVCIdxs.includes(idx))
    idx = Math.floor(Math.random() * pss.vcs.length);
  return idx;
};
...
export const getPassesNewPSSSamePass = (
  ...
  const vcIdx: number = Math.floor(Math.random() * newVCs.length);
